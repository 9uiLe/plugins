...
- **second-opinion** (v0.3.0): ツール非依存の `decision-council` Skill を追加。証拠 readiness gate、Fable/Codex 等の独立 first round、匿名 challenge、全提案の disposition、議長・役割別モデル/effort 選定、劣化時の confidence 上限、意思決定記録を定義した。AWS sample Collaborative AI-DLC の Inception/traceability/role separation/human gate/refinement loop を一次資料付きで取り込み、Orca/orchestration は任意 transport に限定した。
