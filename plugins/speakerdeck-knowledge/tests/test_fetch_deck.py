...
class FetchDeckTests(unittest.TestCase):
    ...
    def test_incremental_cli_preserves_state_and_only_retries_requested_failures(self):
        jpeg = b'\xff\xd8\xff' + b'image-bytes'
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / 'source.html'
            source.write_text(fixture(), encoding='utf-8')
            command = [str(SCRIPT), URL, '--html-file', str(source), '--out', str(root)]

            def run(*args):
                output = io.StringIO()
                with patch.object(sys, 'argv', command + list(args)), redirect_stdout(output):
                    status = fetch.main()
                return status, json.loads(output.getvalue())

            with patch.object(fetch, 'fetch', side_effect=[jpeg, ValueError('HTTP 403')]):
                status, summary = run('--images', 'all')
            self.assertEqual(status, 2)
            self.assertEqual(summary['images_requested'], 2)
            self.assertEqual(summary['images_failed'], [2])
            original = root / 'slide-001.jpg'
            ...
            self.assertEqual(summary['images_failed'], [])
            saved = json.loads((root / 'deck.json').read_text())
            self.assertEqual([p['image_status'] for p in saved['pages']], ['downloaded', 'failed'])
            self.assertIn('403', saved['pages'][1]['image_error'])

            with patch.object(fetch, 'fetch', return_value=jpeg) as request:
                status, summary = run('--images', '1-2')
            request.assert_called_once_with(saved['pages'][1]['image_url'])
            self.assertEqual(status, 0)
            self.assertEqual(summary['images_requested'], 2)
            self.assertEqual(summary['images_failed'], [])
            saved = json.loads((root / 'deck.json').read_text())
            self.assertEqual([p['image_status'] for p in saved['pages']], ['downloaded'] * 2)
            self.assertNotIn('image_error', saved['pages'][1])
            self.assertEqual(original.read_bytes(), jpeg)
            self.assertEqual(original.stat().st_mtime_ns, modified)

    def test_cache_requires_matching_source_and_nonempty_local_file(self):
        jpeg = b'\xff\xd8\xff' + b'image-bytes'
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / 'evidence'
            root.mkdir()
            (root.parent / 'slide-001.jpg').write_bytes(jpeg)
            ...
            for change in ['url', 'missing', 'empty', 'unsafe_path']:
                with self.subTest(change=change):
                    (root / 'slide-001.jpg').write_bytes(jpeg)
