...
保存先を指定すると、その場所に成果物を作成します。指定ディレクトリに追加の階層は付けません。相対パスはスキル実行開始時の作業ディレクトリを基準にします。

未指定の場合は、作業ディレクトリの `ai-knowledge/` 配下にスライドごとのディレクトリを作成します。名前には URL の最後のパス要素を使用します。
...
このリポジトリの `examples/` が、通常利用時の `ai-knowledge/` に相当します。形式を指定した場合は指定形式だけを作成します。検証記録や配布用画像を作成する場合も同じ資料のディレクトリにまとめ、取得キャッシュは一時ディレクトリに保存します。同じ資料の再生成には同じ保存先を使い、別資料と名前が重なる場合は著者の識別子を付けて区別します。
