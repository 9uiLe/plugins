...
> 結合の **3 次元モデル (Integration Strength × Distance × Volatility)** と Connascence による深掘り語彙は補論 [`07a-coupling-deep-dive.md`](./07a-coupling-deep-dive.md) を参照。**設計時（コードが無い段階）にモジュール境界の結合を検討する場合は補論 §11**、レビュー時の SIGNAL ベース運用は補論 §6.5 と [`07a-review-integration.md`](./07a-review-integration.md) の Merge Contract を参照。本ファイルのしきい値・PASS/FAIL は補論の知見によって **上書きされない**（補論 §9 H9: 重大度は下方修正のみ可）。
