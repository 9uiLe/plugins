...
`quality-review` が「設計そのものを作り直すべき（design-level 指摘が支配的）」と結論し、ユーザが**置換アーキの新規設計**を望む場合は `quality-architecture` へ進む。これは §5.1 ルール 1 がブロックする**後方向**（既存コード対象を architecture で受ける）とは別物で、**再設計という新規設計タスクへの正当な遷移**。逆向き（architecture で既存コードのレビューを処理）は引き続き禁止。
