...
TOOL_MAP_FILE=""
# 早期 exit (空出力・ERR trap 経由) でも一時マップを残さないよう EXIT で回収する
trap 'rm -f "${TOOL_MAP_FILE:-}" 2>/dev/null || true' EXIT
build_tool_map() {
  # tool_result 行は tool 名を持たないため、transcript 全体から id→名 の対応表を先に作る
  TOOL_MAP_FILE=$(mktemp "${TMPDIR:-/tmp}/compact-plus-toolmap.XXXXXX") # lint:allow-os-tmp
  jq -r '.message.content[]? | objects | select(.type? == "tool_use") | [.id, .name] | @tsv' \
    "$TRANSCRIPT_PATH" 2>/dev/null > "$TOOL_MAP_FILE" || true
}

lookup_tool_by_result_id() {
  local json="$1" id
  id=$(jq_string '[.message.content[]? | objects | select(.type? == "tool_result") | .tool_use_id?]
    | map(select(. != null and . != "")) | first // ""' "$json")
  [[ -n "$id" && -f "${TOOL_MAP_FILE:-}" ]] || return 0
  awk -F'\t' -v id="$id" '$1 == id { print $2; exit }' "$TOOL_MAP_FILE"
}
...
extract_text() {
  ...
  jq_string '
    ...
    [
      ...
      (.toolUseResult? | if type=="string" then . elif type=="object" then (.stdout? // .text? // (.content? | if type=="array" then ([.[]? | if type=="object" then (.text? // "") else tostring end] | join("\n")) elif type=="string" then . else "" end) // "") else "" end),
      (.message.content[]? | objects | select(.type? == "tool_result") | .content? | if type=="array" then ([.[]? | if type=="object" then (.text? // "") else tostring end] | join("\n")) elif type=="string" then . else "" end)
...
process_json_line() {
  ...
  [[ -n "$tool" ]] || tool=$(lookup_tool_by_result_id "$json")
  ...
  case "$tool" in
    ...
    Bash)
      if [[ "$char_count" -gt "$COMPACT_PLUS_SQUASH_BASH_CHARS" ]]; then
        exit_code=$(jq_string '.exit_code // .status // .metadata.exit_code // .toolUseResult.exitCode? // .toolUseResult.exit_code? // "unknown"' "$json")
...
build_user_prompt() {
  ...
  if state_is_valid && { [[ "$mode" == "incremental" || "$mode" == "refresh" ]]; }; then
    cat "$STATE_FILE"
  else
    printf '(none)\n'
  fi
...
run_backend_if_set() {
  ...
  if output=$(SYSTEM_PROMPT="$SYSTEM_PROMPT" SESSION_ID="$SESSION_ID" TRANSCRIPT_PATH="$TRANSCRIPT_PATH" MAX_OUTPUT_TOKENS="$COMPACT_PLUS_MAX_OUTPUT_TOKENS" bash -c "$cmd" <<< "$user_prompt" 2>/dev/null); then
    if [[ "$(printf '%s\n' "$output" | head -n 1)" == "# Compact Prep State" ]] && required_headings_ok "$output"; then
      printf '%s\n' "$output"
      return 0
    fi
  fi
