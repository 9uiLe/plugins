...
- compact-plus は warn marker を書く statusline 表示自体を強制しない。同梱の `scripts/compact-warn-statusline.sh` は `settings.json` の `statusLine` に配線して初めて動くオプトイン実装であり、配線しなければ reminder hook (`userpromptsubmit-compact-plus-reminder.sh`) は marker 不在のまま何もしない (fail open)。
...
3. `precompact-state-summary.sh` が設定済み mode に従って transcript を読む。
   - `incremental`: 前回 offset 以降の new bytes を読み、一定周期 (`COMPACT_PLUS_INCREMENTAL_REFRESH`) で既存 state を保持したまま head/tail からの再サンプリングに切り替え、差分読みで生じたドリフトを補正する。
...
10. statusline warning marker があれば、`userpromptsubmit-compact-plus-reminder.sh` が consume し、compact suggestion と短い state recitation を注入する。statusline warning marker は `scripts/compact-warn-statusline.sh` を `statusLine` に配線している場合のみ生成される任意ステップであり、配線していなくても 1〜9 の経路には影響しない。
...
| `${TMPDIR:-/tmp}/claude-compact-warn/<session_id>` | `scripts/compact-warn-statusline.sh` (`statusLine` に配線した場合のみ) | `userpromptsubmit-compact-plus-reminder.sh` | Threshold warning。compact-plus が producer script を同梱するが、配線は利用者のオプトイン |
...
| `${TMPDIR:-/tmp}/claude-active-plan/<session_id>` | plan-management hook | recovery hook | Active plan pointer。compact-plus は producer を所有しない。外部の plan 管理 hook が書く場合のみ利用される任意マーカーで、存在しなくても compact-plus の動作には影響しない |
...
| `COMPACT_PLUS_INCREMENTAL_REFRESH` | Refresh cadence (state-preserving head/tail resample, not a full rebuild) |
...
| `COMPACT_WARN_THRESHOLD` | Warn statusline threshold percentage, read by the bundled `scripts/compact-warn-statusline.sh` |
| `COMPACT_PLUS_STATUSLINE_DELEGATE` | Optional command that `scripts/compact-warn-statusline.sh` delegates display to |

`COMPACT_WARN_THRESHOLD` と `COMPACT_PLUS_STATUSLINE_DELEGATE` は、いずれも `scripts/compact-warn-statusline.sh` を `statusLine` に配線した場合にのみ評価される。配線していないセッションではこれらの env var は参照されない。
